...
findTTitle []                    _     ti = brackets $ hcat ti
findTTitle ((t, VTArray v) : xs) True  _  = ppTArray v t -- $$ findTTitle xs True ti
...
ppTArray v t = vcat $ map ((\ x -> dBrackets pt $$ findTTitle x False [pt]) . M.toList) (V.toList v)
